...
- **Портативный режим (True Portable Architecture & Zero-Install):**
  ...
  - Портативная структура папки `Portable-L-MPV/`:
    ...
    - `logs/` — локальные журналы работы и диагностика (`mpv.log`, `crash.log`).
...
- **Эргономичная интеграция в блоке «Аудиодорожки и субтитры»:**
  ...
  5. *Папка для извлечения дорожек*: выбор директории для экспорта аудио/субтитров в 1 клик (сохранение рядом с видеофайлом либо запрос пути через диалог).
- **Мгновенный отклик и синхронизация:**
  Каждое изменение настройки моментально сохраняется в `localStorage` (`l-mpv-show-track-names`, `l-mpv-hotload-enabled`, `l-mpv-save-tracks-video-dir`) и рассылает событие `l-mpv-settings-changed`, обеспечивая синхронную реакцию нижней панели управления, всплывающих меню и пресетов.
