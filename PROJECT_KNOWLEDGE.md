...
- Режимы работы:
  - **`Off`**: классические черные полосы (`border-background=color`, `background-color=#000000`, `corner-rounding=0.0`).
...
- **Нативное аппаратное скругление углов (`corner-rounding`):** настраиваемое сглаживание переходов между видеокадром и фоновой подсветкой от 0% до 40% (0.0..1.0 в шейдере `libplacebo`).
- **Архитектурная оптимизация**: инкапсулированный `AmbientController` с кэшированием последнего состояния (исключает повторную отправку одинаковых свойств mpv) и разделением на 60fps GPU preview без дискового I/O и отложенное сохранение (Debounce 400ms).
...
- Регистрирует все **55 IPC-команд** в `tauri::Builder`.

---

## 4. Полный Реестр IPC-Команд Rust (`commands.rs`)

В бэкенде зарегистрирована **55 IPC-команд**:
...
| **Подсветка полос (Ambient Light)** | `get_ambient_settings` | — | `Result<AmbientSettings, String>` | Получение текущего режима, радиуса размытия, скругления углов и цвета подсветки |
