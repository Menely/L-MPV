...
- **Нативный и безопасный MarkdownRenderer (`MarkdownRenderer.tsx`):**
  - Разбор и стилизованный рендеринг заголовков (`#`, `##`, `###`, `####`), списков, чекбоксов (`[x]`/`[ ]`), цитат, блоков кода и инлайн-элементов без использования небезопасного `dangerouslySetInnerHTML`.
  - Полноценная поддержка изображений из релиза (`![alt](url)`) со скруглением, теневыми акцентами, открытием оригинала в браузере по клику и обработкой сетевых сбоев.
...
- **Адаптивное разворачивание окна:**
  - Кнопка `Maximize2`/`Minimize2` в заголовке модалки и поддержка разворачивания по двойному клику по шапке.
  - Плавное динамическое расширение ширины окна (до `820px`) и высоты блока изменений (до `58vh` / `320px`) для удобного чтения объемных списков нововведений.
