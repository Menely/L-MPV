...
Данный документ содержит полную и актуальную информацию об устройстве медиаплеера **L-MPV** (версия **2.0.0**), его архитектуре, технологическом стеке, структуре файлов, взаимодействии Rust и React, детальном реестре IPC-команд, подсистеме Real-Time 4K AI Upscaling и всех пользовательских возможностях.
...
- **`hardware.rs` (Аппаратный Анализ GPU через Win32 DXGI C-API):**
  ...
  - Сопоставление PCI Device ID NVIDIA с версиями Compute Capability / Shader Model (SM 6.0 Pascal, SM 7.0/7.5 Turing, SM 8.0/8.6/8.9 Ampere/Ada Lovelace, SM 10.0+ Blackwell).
...
- **`controller.rs` (Управление Инференсом libmpv и Фоновая Компиляция):**
  ...
  - Неблокирующая фоновая компиляция `.engine` под 1080p через `trtexec.exe` / `aji_harness.exe` с перенаправлением вывода в файл лога сборки.
  - Построчный мониторинг фаз сборки TensorRT 11 (15% -> 28% -> 48% -> 68% -> 90% -> 100%).
  - Физическая валидация размера сгенерированного `.engine` файла (`metadata.len() > 0`) перед отправкой события успешного завершения.
...
- **Предварительная компиляция TensorRT под 1080p (.engine):**
  - Сборка нативного оптимизированного `.engine` файла из `.onnx` модели под разрешение 1080p -> 4K (память воркспейса 4096 МБ, optimization level 3) через `trtexec.exe` или `aji_harness.exe`.
