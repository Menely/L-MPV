...
Данный документ содержит полную и актуальную информацию об устройстве медиаплеера **L-MPV** (версия **2.5.4**), его архитектуре, технологическом стеке, структуре файлов, взаимодействии Rust и React, детальном реестре IPC-команд, подсистеме Real-Time 4K AI Upscaling и всех пользовательских возможностях.
...
- **Эргономичный Редизайн Вкладки «Внешний вид» (AppearanceSettingsTab):**
  - *Прозрачность интерфейса*: горизонтальная компактная плашка вверху со слайдером от 20% до 100%.
  - *Сбалансированная трёхколоночная сетка*:
    1. **Скругление углов**: 4 карточки пресетов (сетка 2x2) + вертикальный ползунок точной настройки пикселей.
    2. **Масштабирование UI Scale**: 4 карточки (сетка 2x2) + вертикальный ползунок + кнопка «A» авто-масштаба в заголовке.
...
- **Интерактивный Drag-and-Drop движок (`@dnd-kit`):**
  ...
  - Прозрачный оверлей перетаскивания `DragOverlay` с `pointer-events: none`, предотвращающий ложные сбросы при дропе.
