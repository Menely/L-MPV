...
Данный документ содержит полную и актуальную информацию об устройстве медиаплеера **L-MPV** (версия **2.5.3**), его архитектуре, технологическом стеке, структуре файлов, взаимодействии Rust и React, детальном реестре IPC-команд, подсистеме Real-Time 4K AI Upscaling и всех пользовательских возможностях.
...
- **Обновление до версии 2.5.3 — Модульная архитектура, палитра 4×4 и персистентность:**
  - **Глубокий рефакторинг подсистемы субтитров (`src/components/subtitles/`):** монолитный файл `SubtitlesSearchModal.tsx` декомпозирован на 14 сфокусированных модулей (сокращение объема главного файла на 72%, с 1386 до 390 строк), выделены отдельные хуки `useActiveLineIndex`, `useFollowPlayback`, `useModalGeometry`, `useSearchNavigation`, `usePersistentState`, а также субкомпоненты строк, шапки, поисковой панели и бейджей.
  - **Адаптивная слежка за речью (Jitter-Free Follow Playback):** интеллектуальный алгоритм `calculateAdaptiveStep` с адаптивным коэффициентом шага прокрутки в зависимости от дистанции: при мгновенном перемотке с 3 на 19 минуту список субтитров плавно ускоряется и центрируется без задержек и микро-дрожания при наведении.
  - **Управление навигацией с клавиатуры:** поддержка клавиш `ArrowUp` / `ArrowDown` для перемещения по репликам и кнопка мгновенного переключения непрозрачности окна субтитров.
  - **Сырой исходный формат ASS и быстрое копирование:** корректное сохранение тегов `\N` без ложного переноса строки и кнопки `[Ht]` (HTML) / `[Ae]` (Aegisub ASS).
  - **Расширение палитры акцентных цветов (Матрица 4×4):** добавлены по 4 новых оттенка в категории «Пастельные» (16) и «Стандартные» (16), лимит «Своих цветов» расширен до 15 (+кнопка Windows = 16), сформирована ровная сетка 4×4.
  - **Отказоустойчивая портативная персистентность (Zero-Data-Loss):** полная аннотация `#[serde(default)]` для всех структур Rust, безопасное частичное восстановление через `serde_json::Value` при повреждении отдельных полей конфига и строгая изоляция данных в локальной папке `config/settings.json` без записи в реестр и системные каталоги.
  - **Исправление обрезания свечения ползунка громкости:** удалено свойство `contain: layout paint;` у `.volume-slider__expandable` и увеличены отступы для свободного рендеринга неонового drop-shadow / glow ореола.
