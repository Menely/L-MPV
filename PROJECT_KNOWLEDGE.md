...
Данный документ содержит полную и актуальную информацию об устройстве медиаплеера **L-MPV** (версия **1.3.1**), его архитектуре, технологическом стеке, структуре файлов, взаимодействии Rust и React, детальном реестре IPC-команд и всех пользовательских функциях плеера.
...
- Регистрирует все **59 IPC-команд** в `tauri::Builder`.

---

## 4. Полный Реестр IPC-Команд Rust (`commands.rs`)

В бэкенде зарегистрирована **59 IPC-команд**:
...
| | `get_auto_load_tracks` | — | `Result<bool, String>` | Получение статуса настройки автоматического подхвата внешних дорожек |
| | `set_auto_load_tracks` | `enabled: bool` | `Result<(), String>` | Включение/выключение автоподхвата внешних дорожек с сохранением в `settings.json` |
...
| **Система и Метаданные** | `get_app_version` | — | `Result<String, String>` | Получение актуальной версии приложения (`CARGO_PKG_VERSION`) |
...
### 5.5. Скриншоты и Настройки (Screenshots & Settings)
- **Создание Скриншотов:**
...
### 5.6. Система автоматического обновления (In-App Auto-Update)
- **Фоновая проверка обновлений (Каждые 5 запусков):**
  - При каждом старте приложения значение `launch_count` в `config/settings.json` увеличивается на 1.
  - Если `launch_count % 5 == 0`, плеер асинхронно отправляет запрос к GitHub Releases API (`https://api.github.com/repos/Menely/L-MPV/releases/latest`).
  - При отсутствии подключения к интернету плеер работает штатно без задержек и предупреждений.
  - При обнаружении более новой версии всплывает компактное окно обновления.
- **Ручная проверка обновлений:**
  - В футере окна «Настройки» расположена кнопка *«Проверить обновления»* с индикатором загрузки и выводом статуса.
- **Интерактивное окно обновления (`UpdateModal.tsx`):**
  - Выдержано в фирменном стиле Glassmorphism с плавной анимацией появления.
  - Отображает сравнение текущей и новой версий (`v1.3.0` → `v1.3.1`), список изменений (Changelog/Release Notes), кнопки «Установить» и «Напомнить позже».
- **Потоковая загрузка и бесшовный запуск инсталлятора:**
  - Скачивание установочного файла (`*setup.exe`) во временную директорию ОС (`%TEMP%`) потоком (Stream Chunks) с трансляцией прогресса загрузки во фронтенд в реальном времени.
  - Запуск инсталлятора NSIS в отдельном независимом процессе с флагом `DETACHED_PROCESS` (`0x00000008`).
  - Мгновенное чистое завершение процесса плеера (`app.exit(0)`), освобождающее все файловые блокировки (`L-MPV.exe`, `libmpv-2.dll`, `ffmpeg.exe`), позволяя установщику беспрепятственно обновить плеер и все сопутствующие бинарники.
