...
Данный документ содержит полную и актуальную информацию об устройстве медиаплеера **L-MPV** (версия **2.5.1**), его архитектуре, технологическом стеке, структуре файлов, взаимодействии Rust и React, детальном реестре IPC-команд, подсистеме Real-Time 4K AI Upscaling и всех пользовательских возможностях.
...
- **Предварительная компиляция TensorRT под 1080p (.engine):**
  - Прямой вызов официального компилятора `trtexec.exe` с параметрами: `--onnx=... --saveEngine=... --builderOptimizationLevel=5 --optShapes=input:1x3x1080x1920 --skipInference --timingCacheFile=...` (полностью идентично логике `libaji`/`aji_trt.cpp:build_engine`).
...
### 5.15. Улучшения и оптимизации версии 2.5.1
- **Переход на лицензию GNU General Public License v3.0 (GPLv3):**
  - Полная юридическая чистота и гармонизация с лицензиями нативных сборок `libmpv-2.dll` (GPLv3) и `ffmpeg.exe`.
  - Добавлено четкое разграничение для нейросетевых моделей апскейлинга AnimeJaNai под лицензией Creative Commons Attribution-NonCommercial-ShareAlike 4.0 International (CC BY-NC-SA 4.0).
- **Адаптивная динамическая привязка субтитров к интерфейсу:**
  - В команду `update_subtitles_avoid_ui` интегрирован учет реальной высоты окна (`window.innerHeight`), решающий проблему фиксированного процента `sub-pos`.
  - Процент `sub-pos` теперь рассчитывается по формуле $100 - (104\text{px} / \text{height} \times 100)$, удерживая субтитры строго на высоте ~104px от нижнего края (на 20–25px выше контрольной панели) как в компактном оконном режиме (540p/720p), так и в полноэкранном 1080p/1440p/4K.
  - Подключены слушатели события `resize` и переключения `fullscreen` для мгновенного бесшовного пересчета смещения на лету.
- **Эргономическая очистка UI от лишних тултипов (`title`):**
  - Удалены мешающие всплывающие подсказки браузера с кнопки проверки обновлений, всех кнопок интеграции с Windows, карточек стиля панели управления и цветовых палитр тем плеера и визуализатора.
