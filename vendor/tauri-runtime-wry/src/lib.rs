...
"#,
  ...
  let mut webview_transparent = webview_attributes.transparent;
  #[cfg(target_os = "linux")]
  ...
  if label == "main"
    && gtk::gdk::Display::default()
      ...
      .unwrap_or(false)
  {
    // X11 cannot alpha-compose WebKitGTK over GtkGLArea. The video surface is
    // an overlay there, so an opaque WebView base keeps controls/title visible
    // in the area outside the video rectangle.
    webview_transparent = false;
  }

  let mut webview_builder = WebViewBuilder::new_with_web_context(&mut web_context.inner)
    .with_id(&label)
    .with_focused(webview_attributes.focus)
    .with_transparent(webview_transparent)
  ...
  let webview = match kind {
    ...
    WebviewKind::WindowChild => {
      ...
      if label == "main" && std::env::var("L_MPV_NATIVE_VIDEO").as_deref() != Ok("0") {
        use gtk::glib::ObjectExt;
        use gtk::prelude::{BoxExt, ContainerExt, OverlayExt, WidgetExt};

        let overlay = gtk::Overlay::new();
        let video = gtk::GLArea::new();
        let webview_host = gtk::Box::new(gtk::Orientation::Vertical, 0);
        video.set_widget_name("l-mpv-video");
        video.set_hexpand(true);
        video.set_vexpand(true);
        webview_host.set_hexpand(true);
        webview_host.set_vexpand(true);
        let is_x11 = gtk::gdk::Display::default()
          .map(|display| display.type_().name() == "GdkX11Display")
          .unwrap_or(false);
        if is_x11 {
          overlay.add(&webview_host);
          overlay.add_overlay(&video);
        } else {
          overlay.add(&video);
          overlay.add_overlay(&webview_host);
        }
        vbox.pack_start(&overlay, true, true, 0);
        webview_builder.build_gtk(&webview_host)
      } else {
