  ...
  pub fn attach_resize_handler(webview: &wry::WebView) {
    ...
    webview.connect_button_press_event(
      move |webview: &webkit2gtk::WebView, event: &gtk::gdk::EventButton| {
        if event.button() == 1 {
          // Custom Linux shells may place the WebView inside GtkOverlay (for
          // example, over a GtkGLArea used by libmpv). Walk the hierarchy
          // instead of assuming WebView -> GtkBox -> GtkWindow.
          let mut ancestor = webview.parent();
          let mut gtk_window = None;
